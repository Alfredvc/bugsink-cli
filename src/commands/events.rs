...
pub async fn run(command: &EventsCommands, output: &Output, url: Option<&str>, token: Option<&str>, all: bool) -> Result<()> {
    ...
    match command {
        ...
        EventsCommands::Stacktrace { id } => {
            let text = client.get_text(&format!("events/{}/stacktrace/", id)).await?;
